...
This document describes the standard release process for airflow-operator.
...
2. **Docker image** — uses `VERSION` directly:
   `quay.io/zncdatadev/airflow-operator:<version>`
...
4. **Helm chart publish** — pushes to
   `quay.io/kubedoopcharts/airflow-operator:<version>` (OCI registry)
...
```bash
gh api repos/zncdatadev/airflow-operator/git/refs \
  -f ref=refs/heads/release-0.x \
  -f sha=$(gh api repos/zncdatadev/airflow-operator/git/ref/heads/main --jq .object.sha)
```
...
- Docker image is available at
  `quay.io/zncdatadev/airflow-operator:x.y.z-dev`
- Helm chart is available at
  `quay.io/kubedoopcharts/airflow-operator:x.y.z-dev`
...
- **Release Image** — Builds and pushes multi-arch Docker image using the
  root Dockerfile to
  `quay.io/zncdatadev/airflow-operator:<version>`, and signs the image with
  Cosign
- **Release Chart** — Publishes the Helm chart to
  `quay.io/kubedoopcharts/airflow-operator:<version>` (OCI registry) and
  updates the
...
airflow-operator follows [Semantic Versioning](https://semver.org/):
